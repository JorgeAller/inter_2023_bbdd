...
const selectFilmsByIdSessionQuery = async (idSession) => {
  let connection;
  try {
    ...
    const [films] = await connection.query(
      ...
      SELECT 
        ...
        TM.id AS team_id,
        PF.people_id AS people_film_id,
        F.idSession,
        F.title, 
        F.short_desc, 
        F.image, 
      ...
      FROM 
        ...
        LEFT JOIN people_films PF ON F.id = PF.films_id
      WHERE 
        F.idSession = ?
...
module.exports = selectFilmsByIdSessionQuery;
